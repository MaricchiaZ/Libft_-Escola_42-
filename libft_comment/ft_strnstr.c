...
//man strnstr:
//A função strnstr()  encontra a primeira ocorrência da substring agulha (terminada 
//por '\0' na string, onde não são pesquisados mais de n caracteres.  Os caracteres que 
...
//a substring não for encontrada. Se a agulha for uma string vazioa, o valor de retorno é 
//sempre o próprio palheiro.

//essa função procura se o conteúdo da string s2 aparece na strinh s1, mas só até n bytes
char	*ft_strnstr(const char *s1, const char *s2, size_t n)
{
	...
	while (str[i] != '\0' && p < n) //enquanto stri não for nula e o índice da to_find for menor que n
	{
		p = 0; // o índice que vai percorrer a to_find é iniciado zerado
		while (str[i + p] == to_find[p] && str[i + p] != '\0' && i + p < n) // enquanto
		...
		//i + p for menor que o número de bites passados por n
		{
